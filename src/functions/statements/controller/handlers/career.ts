...
*/
import { User, Career } from 'pal-mongoose';
import { StatementHandler } from './index';
...
const objectFinder = new StatementObjectFinder<Career>({
  iriToId: /.*\/careers\/([^/?]+)/,

  findById: (id) => Career.findById(id).exec(),
});

export default class CareerHandler implements StatementHandler {
  async handleStatements(
    ...
    for (let i = 0; i < targetObjects.length; i++) {
      ...
      result[i] = setObjectName(object.alias, s);
    }
    return result;
  }
}
